...
- **便签**　碎片记录，多列便签墙，卡内直接编辑 Markdown 原文（可省标题），长内容用预览弹窗看渲染成品；不分组、不标周次，置顶优先、新的在前。
...
`/notes`。等宽多列网格，按行排列、列数随窗口宽度自适应；三种便签纸色，点击卡片直接写标题与 Markdown 原文，「预览」按钮把渲染结果开在弹窗里，顶部搜索（标题 + 正文）本地过滤。
