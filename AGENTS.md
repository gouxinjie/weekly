...
### 红线 3：时间轴起点固定 2025，服务端强制校验

起点是服务端常量 `START_YEAR = 2025`，**不可从用户注册时间推导**。

```ts
// 用户 A 2025 注册、用户 B 2027 注册
// ❌ 错误：按注册年推导，B 永远看不到 2025 年
const startYear = user.registeredAt.getFullYear();
// ✅ 正确：服务端常量
const START_YEAR = 2025;
```

服务端必须**拒绝**任何早于 2025 年第 1 周的写入，不能只靠前端不显示。

**⚠️ 这里有个真实的实现陷阱**（已用 Node 实测验证）：

| 事实 | 值 |
|---|---|
| 2025 年第 1 周的周一是 | **2024-12-30**（日期落在 2024 年！） |
| 2025 年总周数 | 52 周 |
...
1. **起点校验要按「周」而不是按「日期」**。用 `year >= START_YEAR` 判断合法性，**不能**写成 `date >= '2025-01-01'`——后者会把合法的 2025 年第 1 周（2024-12-30 起）误判为越界。

2. **周次上限是 53，不是 52**。校验、循环、范围生成都要用 53。写死 52 会丢掉每年的最后一周（2026 年就有 53 周）。
...
:root {
  ...
  --note-create-width:   520px;  /* 新建便签弹窗：宽度（写几行正文的舒适档） */
  ...
  /* 移动端（断点 1023px，断点值在样式里以字面量出现，见 §6.1） */
  --mobile-tabbar-height: 52px;   /* 底部页签栏高度（安全区另加 env(safe-area-inset-bottom)） */
  --mobile-nav-button:    32px;   /* 抽屉入口按钮的高度 */
  --mobile-drawer-width:  300px;  /* 左列抽屉宽度上限（实际取 min(82vw, 该值)） */
  --mobile-side-width:    340px;  /* 右栏浮层宽度上限（实际取 min(86vw, 该值)） */
  --mobile-gutter:        16px;   /* 移动端页面横向留白 */
...
- **二选一的类名必须把共有样式挂在两个类上**。`className={on ? styles.aOpen : styles.a}` 这类写法下，元素身上永远只有其中一个类；若把底色、`display`、`flex-direction` 这些共有样式只写在 `.a` 上，切到 `.aOpen` 时会整套丢失——**浏览器不报任何错**，只表现为元素突然透明、布局塌陷，排查时容易误判成 z-index 或层级问题。正确写法是 `.a, .aOpen { 共有样式 }`，再各自写差异。已有实例：`.leftColumn` / `.leftColumnOpen`（抽屉展开后背景透明，已修）、`.tab` / `.tabActive`、`.avatar` / `.avatarOpen`、`.text` / `.textDone`
...
| 中栏 | 自适应（最小 480px）：顶栏 + 标题栏 + 编辑区 + 工具条 | 自适应（最小 480px）：标题 + 搜索 + 新建输入框 + 分组清单 | 自适应（最小 480px）：标题 + 搜索 + 新建 + 等宽多列网格的便签卡片 |
...
断点约定：**≥ 1024px 走桌面骨架，≤ 1023px 走移动骨架**。样式侧统一写 `@media (max-width: 1023px)`（CSS 变量不能参与媒体查询条件，所以断点值在各文件里以字面量出现）；脚本侧统一用 `constants.MOBILE_MEDIA_QUERY`（经 `hooks/useIsMobile` 暴露），两边不能各写一套数字，否则会出现「脚本以为在移动端、样式以为在桌面」的错位。

- DOM 与桌面端**完全一致**，只在断点内改排布——不允许为了移动端另起一套组件树，那样两边的交互会慢慢分叉
- 页签栏从左侧竖排落到屏幕底部：四个页签等宽横排，品牌区 `display: none`，账号区只留一枚头像（账号名与手机号在设置页看）
- 左列与右栏脱离文档流，成为覆盖式抽屉（在 `.contentRow` 里 `position: absolute`，收起态用 `translateX(-100%)` 移出屏幕）：由入口按钮唤起、`.backdrop` 点击收起，两者同时只开一个
- 入口按钮 `.mobileNavButton` 由骨架统一渲染，摆放位置跟着页面走：有顶栏的页面（周报）由骨架摆进顶栏最前面；没有顶栏的页面（待办）用骨架导出的 `MobileDrawerEntry` 取走、放进自己的标题行。**不要为了一枚按钮另起一条工具条**——那会白占一行高度，视觉上还多切出一块
...
**不要写成 `new Date(...) >= '2025-01-01'`**——2025 年第 1 周的周一是 **2024-12-30**，按日期判断会误拒合法数据（详见红线 3）。
...
- **错误反馈**：表单校验与保存失败**就地显示**；仅跨区域短反馈（导出成功、登出所有设备、便签创建成功）用顶部 Toast
...
- **新建便签走弹窗**（`components/NoteCreateDialog`）：「＋ 新建」只打开弹窗，标题与正文在弹窗里一次填好再创建；**创建失败时弹窗保持打开**、错误就地提示，已填内容不丢；创建成功关闭弹窗并用顶部 **Toast 回执**（弹窗一关，操作与结果就不在同一处了）；两个字段都为空即得到一张空白便签并自动聚焦，已写好内容的便签不抢焦点。空白便签的丢弃判定仍按上一条走
- **便签不与周报 / 待办互转**：模块之间不做任何桥接，便签只进不出
- **多标签页**：仅检测并提示刷新，**不做 WebSocket 实时同步**
- **移动端**：视口 ≤ 1023px 时切换为移动端骨架（底部页签栏 + 覆盖式抽屉，见 §6.1），桌面端布局与交互不变；触摸设备上 HTML5 拖拽不生效，待办排序改走「⋯」菜单里的上移 / 下移；左右滑动切周、下拉刷新等手势不做
...
| 提交 2024 年第 52 周 | 服务端拒绝（早于起点年） |
| **提交 2025 年第 1 周（周一为 2024-12-30）** | **成功**——不能因日期在 2024 年而误拒 |
