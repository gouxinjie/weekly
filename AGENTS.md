...
2. 左列两态都在且同宽（220px），内容不同：周报态渲染时间轴树，待办态渲染筛选列表；两态都与中栏共享顶栏
...
**⚠️ `foreign_keys` 默认是关闭的**，不显式开启会导致删除用户时其周报、待办、会话不级联删除，变成孤儿数据。
...
集中封装在 `web/src/api/client.ts`，业务接口按模块拆分（`weekly.ts` / `todo.ts`）。每个接口函数必须有中文 JSDoc。
